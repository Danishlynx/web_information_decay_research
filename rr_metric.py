...
class ReplayabilityRatioAnalyzer:
            ...
            if response:
                ...
                if screenshot_path.exists():
                    ...
                    if (200 <= result['status_code'] < 300 and 
                        ...
                        result['rr_score'] = 1.0
                        logger.info(f"[SUCCESS] {category.upper()} page successfully replayed: {url}")
                    else:
                        ...
                        logger.warning(f"[PARTIAL] {category.upper()} page partial replay: {url} (score: {partial_score:.2f})")
            else:
    ...
    def calculate_statistics(self) -> Dict:
        """Calculate detailed statistics by category."""
        if not self.results:
            return {}
        ...
        stats = {
            'overall': {
                ...
                'average_rr': sum(scores) / len(scores) if scores else 0,
                'success_rate': sum(1 for s in scores if s >= 0.8) / len(scores) * 100 if scores else 0
            },
            'by_category': {}
        }
        ...
        for cat in categories:
            ...
            if cat_scores:
                stats['by_category'][cat] = {
                    ...
                    'average_rr': sum(cat_scores) / len(cat_scores),
                    'success_rate': sum(1 for s in cat_scores if s >= 0.8) / len(cat_scores) * 100,
                    'avg_load_time': np.mean([r['load_time'] for r in cat_results if r['load_time'] > 0]),
                    'avg_dom_elements': np.mean([r['dom_elements'] for r in cat_results if r['dom_elements'] > 0])
                }
    ...
    def create_visualizations(self, stats: Dict):
        ...
        colors = ['#2E7D32', '#1976D2', '#F57C00', '#7B1FA2']  # Different color for each category
        ...
        bars = ax3.bar(categories, avg_scores, color=['#2E7D32', '#1976D2', '#F57C00', '#7B1FA2'], 
        ...
        # Prepare table data
        table_data = [['Category', 'Total', 'Success', 'Partial', 'Failed', 'Avg Score', 'Success Rate']]
        for cat in categories:
        ...
        table_data.append([
            'OVERALL',
            str(overall['total_urls']),
            str(overall['successful_replays']),
            str(overall['partial_replays']),
            str(overall['failed_replays']),
            f"{overall['average_rr']:.3f}",
...
async def main():
    ...
    for cat, cat_stats in stats['by_category'].items():
        ...
        print(f"    • Avg DOM Elements: {int(cat_stats['avg_dom_elements'])}")
    
    # Key findings based on dissertation objectives
    print(f"\n🔍 KEY FINDINGS (aligned with dissertation objectives):")
    
    # Find best and worst performing categories
    best_cat = max(stats['by_category'].items(), key=lambda x: x[1]['success_rate'])
    worst_cat = min(stats['by_category'].items(), key=lambda x: x[1]['success_rate'])
    ...
    # Calculate performance degradation for SPAs vs HTML (as per dissertation hypothesis)
    if 'html' in stats['by_category'] and 'spa' in stats['by_category']:
    ...
    if 'social' in stats['by_category']:
        social_fail_rate = (stats['by_category']['social']['failed'] / 
                           stats['by_category']['social']['count'] * 100)
