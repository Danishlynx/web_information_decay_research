...
class VisualCorrespondenceAnalyzer:
    ...
    async def measure_vcs(self, url: str, archive_url: str, category: str, context) -> Dict:
        ...
        try:
            ...
            live_screenshot, archive_screenshot = await self.capture_screenshots(url, archive_url, category, context)
    ...
    async def analyze_urls(self, urls: List[Dict]) -> List[Dict]:
        ...
        try:
            for idx, url_data in enumerate(urls, 1):
                ...
                if not archive_url:
                    print(f"  No archive found - recording full damage (0.0 VCS)")
                    self.results.append({
                        'url': url,
                        'category': category,
                        'vcs_score': 0.0,
    ...
    def calculate_and_visualize(self):
        ...
        for cat in categories_all:
            ...
            scores = [r['vcs_score'] for r in cat_results if r['vcs_score'] > 0]
            if scores:
                stats[cat] = {
                    ...
                    'count': len(scores),
                    'no_archive_count': sum(1 for r in cat_results if not r['has_archive'])
                }
            else:
                stats[cat] = {
                    'avg_vcs': 0.0,
        ...
        for cat, s in stats.items():
            ...
            print(f"  No archive count: {s['no_archive_count']}")
